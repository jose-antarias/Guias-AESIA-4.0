...
export function GuideGlossary({ id }: { id: string }) {
            ...
            return (
                <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white">
                    <div className="text-center">
                        ...
                        <p className="mb-6 text-slate-400">Esta guía aún no tiene términos definidos.</p>
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-indigo-500/30">
            {/* Header Sticky */}
            <header className="sticky top-0 z-50 border-b border-indigo-500/10 bg-slate-950/80 backdrop-blur-md">
                <div className="container mx-auto px-4 h-16 flex items-center justify-between max-w-5xl">
                    ...
                    >
                        <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center mr-3 group-hover:bg-indigo-500/20 group-hover:text-indigo-400 transition-colors">
            ...
            <main className="container mx-auto px-4 py-12 max-w-4xl">
                ...
                <div className="text-center mb-12">
                    <h1 className="text-3xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-indigo-400 mb-4 animate-gradient-text pb-2">
                        {t.glossary.title}
                    </h1>
                ...
                <div className="relative max-w-xl mx-auto mb-16 group">
                    <div className="absolute inset-0 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full blur opacity-20 group-hover:opacity-40 transition-opacity" />
                    <div className="relative flex items-center bg-slate-900 border border-slate-700 focus-within:border-indigo-500 rounded-full px-4 py-3 shadow-xl transition-all">
                        ...
                        />
                        {searchTerm && (
                            <button onClick={() => setSearchTerm("")} className="ml-2 text-slate-500 hover:text-white">
                                <X className="w-4 h-4" />
                            </button>
                        )}
                    </div>
                        ...
                        sortedLetters.map((letter) => (
                            <div key={letter} className="relative">
                                <div className="sticky top-20 z-10 flex items-center mb-6">
                                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-700 flex items-center justify-center shadow-lg font-black text-2xl text-white border border-white/10">
                                        {letter}
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-0 md:pl-16">
                                    {groupedTerms[letter].map((item, idx) => (
                                        <motion.div
                                            ...
                                            key={item.term}
                                            className="bg-slate-900/50 border border-slate-800/60 p-5 rounded-xl hover:border-indigo-500/30 hover:bg-slate-800/40 transition-all group"
                                        >
                                            <h3 className="text-lg font-bold text-indigo-300 mb-2 group-hover:text-indigo-200 transition-colors">
                                                {item.term}
                                            </h3>
